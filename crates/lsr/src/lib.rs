...
fn find_files(paths: &[String], show_hidden: bool) -> MyResult<Vec<PathBuf>> {
    ...
    for name in paths {
        ...
        // metadata的使用方法
        // 1. metadata::is_dir -> bool, 判断条目是否为目录
        // 2. metadata::mode -> u32 返回权限对应的10进制代表制
        // 3. metadata::nlink -> 找到文件的连接数
        // 4. metadata::uid -> userId
        //    metadata::gid -> groupid
        //    这两个id都必须通过get_user_by_uid/gid获得真实字符串
        // 5. metadata::len -> 返回文件或者目录的大小
    }
    Ok(results)
}
...
fn format_output(paths: &[PathBuf]) -> MyResult<String> {
    let fmt = "{:<}{:<}  {:>}  {:<}  {:<}  {:>}  {:<}  {:<}";
    let mut table = Table::new(fmt); // 创建一个表格
    for path in paths {
        ...
        table.add_row(
            Row::new()
                .with_cell(file_type) //1. d -> 目录， - 其他类型
                .with_cell(perms) //2. 用户，组，其他用户的权限 用十位字符串表示，首位表示条目类型
                .with_cell(metadata.nlink()) //3. 指向该文件的连接数
                .with_cell(user) //4. 用户名
                .with_cell(group) //5. 组名
                .with_cell(metadata.len()) //6. 大小
                .with_cell(modified) //7. 修改日期
                .with_cell(path.display()), //8. 路径名称
        );
    }
    Ok(format!("{}", table))
}
